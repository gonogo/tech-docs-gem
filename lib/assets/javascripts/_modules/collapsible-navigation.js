(function ($, Modules) {
  ...
  Modules.CollapsibleNavigation = function () {
    ...
    this.start = function ($element) {
      ...
      collapsibleHeadings()
      openActiveHeading()
      // Page scroll is on the window (sticky TOC layout); keep listening in case
      // a consumer still scrolls .app-pane__content independently.
      $(window).on('scroll', _.debounce(openActiveHeading, 100, { maxWait: 100 }))
    ...
    // Treat /foo/, /foo and /foo/index.html as the same location (clean URLs vs
    // relative_links sidebar hrefs).
    ...
    function openActiveHeading () {
      ...
      for (var i = $topLevelItems.length - 1; i >= 0; i--) {
        ...
        if (pathsMatch(getAbsolutePath($heading), currentPath)) {
        ...
        var $matchingChildren = $children.filter(function (_) {
          return pathsMatch(getAbsolutePath($(this)), currentPath)
        })
